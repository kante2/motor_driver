...
class KmcHardwareDriverDemoNode_ver2 final : public rclcpp::Node {
public:
  KmcHardwareDriverDemoNode_ver2() : Node("kmc_hardware_driver_demo_node") {
    // CAV_ID 환경변수 읽기
    const char* cav_id_env = std::getenv("CAV_ID");
    ...
    cmd_sub_ = create_subscription<geometry_msgs::msg::Twist>(
        cmd_vel_topic, 10, [this](geometry_msgs::msg::Twist::SharedPtr msg) {
  ...
  ~KmcHardwareDriverDemoNode_ver2() override { driver_.stop(); }
...
int main(int argc, char** argv) {
  ...
  try {
    auto node = std::make_shared<KmcHardwareDriverDemoNode_ver2>();
    rclcpp::spin(node);
